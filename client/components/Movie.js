...
class Movie extends Component {
  ...
  getSimilar(id) {
    var th = this;
    axios.get(Remote("movie/" + id + "/similar")).then(function (result) {
      th.setState({
  ...
  getCredits(id) {
    var th = this;
    axios.get(Remote("movie/" + id + "/credits")).then(function (result) {
      th.setState({
        credits: result.data,
      });
    });
  ...
  getImages(id) {
    ...
    axios
      .get(
        Remote("movie/" + id + "/images", { include_image_language: "en,null" })
      )
  ...
  getAccountStates(id, sessionId) {
    var th = this;
    axios
      .get(Remote("movie/" + id + "/account_states", { session_id: sessionId }))
      .then(function (result) {
  ...
  render() {
    ...
    if (this.state.credits.crew.length > 0) {
      director = this.state.credits.crew.find(function (element) {
        ...
        <div className="movie">
          ...
          <header className="movie__header">
            ...
            <div className="movie__sidebar__section">
              <h3>Release date</h3>
              <p className="movie-list__item__year">
              ...
              <div className="row">
                <div className="medium-12 columns">
                  <div>
                    ...
                    <Rating rating={rated} movieid={movie.id} />
                  </div>
                  <ul className="stats">
                    ...
                    <li className="stats__item">
                      <span className="stats__item__count">
                        {movie.revenue.toLocaleString("en-EN", {
                          style: "currency",
                          currency: "USD",
                      ...
                      {this.state.credits.cast.map(function (cast) {
                        return (
                          <li
                            ...
                            className="cast__list__item scroll__list__item"
                          >
                            <span className="cast__list__item__image">
                              {cast.profile_path != null && (
                                <img
                                  ...
                                  alt={cast.name}
                                  className="cast__list__item__profile"
                                />
                              )}
                            </span>
                      ...
                      {this.state.similar.map(function (movie) {
                        return (
                          <li
                            ...
                            className="movie-list__item scroll__list__item"
                          >
          ...
          <aside className="movie__media">
            <ul className="media__list">
              {this.state.images.backdrops.map(function (image) {
                return (
                  <li
                    key={image.file_path.toString()}
                    className="media__list__item"
                  >
                    {image.file_path != null && (
                      <img
                        ...
                        alt={movie.title}
                        className="media__list__item__image"
                      />
                    )}
                  </li>
                );
    ...
    return <div className="container">{movie_html}</div>;
  }
}
